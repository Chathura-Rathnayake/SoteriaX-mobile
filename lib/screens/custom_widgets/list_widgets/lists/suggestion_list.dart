...
import 'package:soteriax/database/get_suggestions_database_services.dart';
...
class ViewSuggestionsList extends StatefulWidget {
  const ViewSuggestionsList({Key? key}) : super(key: key);

  @override
  _ViewSuggestionsListState createState() => _ViewSuggestionsListState();
}

class _ViewSuggestionsListState extends State<ViewSuggestionsList> {
  @override
  Widget build(BuildContext context) {
    // String d='2021-09-12 14:03';
    // print(DateTime.parse(d).toString());
    return StreamBuilder<QuerySnapshot?>(
        stream: GetSuggestionsDB().getAllSuggestions,
        builder: (context, snapshot) {
          if(snapshot.hasData){
            ...
            return Container(
              child: Text('There Seems to be no previous suggestions'),
            );
          }
        }
    );
