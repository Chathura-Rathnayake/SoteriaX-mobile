  ...
  void initState() {
    ...
    trainingTimeChecker = Timer.periodic(Duration(seconds: 5), (timer) {
      ...
      if(trainingTimeStamp!=null){
        ...
        if(currentTimeStamp.seconds-trainingTimeStamp!.seconds>=0){ //no difference training time has come
          setState(() {
            isTrainingTime=true;
          });
        }
                  ...
                  switch(snapshot.connectionState){
                    ...
                    default:
                      if(snapshot.hasError){
                        trainingTimeChecker?.cancel(); //no need for checking the training time no training
                          ...
                          return Padding(
                            ...
                            child: Column(
                              children: [
                                ...
                                SizedBox(height: 5),
                                if(roleNo==3) //only show if the user is the mobile handler only that assigned user can engage
                                  StreamBuilder<DocumentSnapshot?>(
                                      stream: TrainingOperationsDBServices().getRpiStatus, //check if rpi online
                                      builder: (context, snapshot) {
                                        if(snapshot.hasData){ //hl doc
                                          var piLastOnlineTime=snapshot.data!.get('piLastOnlineTime');
                                          if(Timestamp.now().millisecondsSinceEpoch-piLastOnlineTime<5000){ //if the last update is within 5sec
                                            if(isTrainingTime){ //if training time has reached
