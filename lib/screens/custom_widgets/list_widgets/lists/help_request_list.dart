...
import 'package:soteriax/database/get_help_requests_database_services.dart';
...
class ViewHelpRequestsList extends StatefulWidget {
  const ViewHelpRequestsList({Key? key}) : super(key: key);

  @override
  _ViewHelpRequestsListState createState() => _ViewHelpRequestsListState();
}

class _ViewHelpRequestsListState extends State<ViewHelpRequestsList> {
  @override
  Widget build(BuildContext context) {
    // String d='2021-09-12 14:03';
    // print(DateTime.parse(d).toString());
    return StreamBuilder<QuerySnapshot?>(
        stream: GetHelpRequestsDB().getAllHelpRequests,
        builder: (context, snapshot) {
          if(snapshot.hasData){
            ...
            return Container(
              child: Text('There Seems to be no previous help requests'),
            );
          }
        }
    );
